...
If enabled, IMUNES will use the given jail path (virtual root - vroot) instead of the default one when running the node.

The default vroot is /var/imunes/vroot directory.

Custom image

If enabled, IMUNES will use the given Docker image (virtual root - vroot) instead of the default one when running the node.

The default vroot is imunes/template Docker image.

External Docker interface
